...
Listing.find({'ageLimit.maxAgeUnit':'MO'})
    .exec(function (err, listings) {
        if (!err) {
            ...
            loopOnArrayWithDelay(listings, 210, 0, function (e, i) {
                ...
                var newAge = listings[i].ageLimit.maxAge / 12;

                Listing.update({
                    _id: listings[i]._id
                }, {$set:{ 'ageLimit.maxAgeUnit':'YR',
                    'ageLimit.maxAge': newAge
                    }}, {
                    upsert: true
                }, function (err, num) {
                    if (err) {
              ...
              // console.log("Old Age " + listings[i].ageLimit.maxAge + " New Age: " + newAge)
